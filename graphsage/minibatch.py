...
class EdgeMinibatchIterator(object):
    ...
    def __init__(self, G, id2idx, 
        ...
        #self.ext_adj = self.construct_test_adj()
        #self.test_adj = self.construct_test_adj()
...
class NodeMinibatchIterator(object):
    ...
    def __init__(self, G, id2idx, 
        ...
        #self.ext_nodes = [n for n in self.G.nodes() if self.G.node[n]['ext']]
    ...
    def construct_adj(self):
        ...
        deg = np.zeros((len(self.id2idx),))
        for nodeid in self.G.nodes():
            if self.G.node[nodeid]['test'] or self.G.node[nodeid]['val']:# or self.G.node[nodeid]['ext']:
                continue
            neighbors = np.array([self.id2idx[neighbor]
                for neighbor in self.G.neighbors(nodeid)
                if (not self.G[nodeid][neighbor]['train_removed'])])
    ...
    def batch_feed_dict_ext(self,id2idx, batch_nodes, lbl_ext,val=False):
        batch1id = batch_nodes
        batch1 = [self.id2idx[n] for n in batch1id]

        labels = lbl_ext# np.vstack([self._make_label_vec(node) for node in batch1id])
        feed_dict = dict()
        feed_dict.update({self.placeholders['batch_size'] : len(batch1)})
        feed_dict.update({self.placeholders['batch']: batch1})
        feed_dict.update({self.placeholders['labels']: labels})
